...
<div v-if="order.status == 2" class="status-3-btn" v-tap="confirm()">确认订单</div>
...
export default {
  ...
  methods: {
    ...
    confirm(){
      this.$http.post(`${Lib.C.mOrderApi}materialOrders/${Lib.M.GetRequest().orderNo}/storeConfirmMaterial`).then((res) => {
          alert("订单已更新！")
          location.reload()
      }, (res) => {
          alert("更新订单失败，请稍后重试")
